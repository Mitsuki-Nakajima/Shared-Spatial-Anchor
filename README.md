...
The calibration system is the core component that aligns each user's local XR tracking space with the shared virtual Makerspace coordinate system. This is essential because users join from different physical locations and need to appear in the same virtual space.

### How Calibration Works

The calibration system solves the spatial alignment problem by creating a transformation matrix that converts local XR coordinates to shared Makerspace coordinates:

1. **Local XR Space**: Each user's headset tracks their position relative to their own physical room
2. **Shared Makerspace Space**: A common virtual coordinate system representing the NYU Makerspace
3. **Calibration Matrix**: A 4x4 transformation that maps positions from local space to shared space

### Calibration Process

#### Step 1: User Positioning
The user needs to position themselves in their physical space at a known reference point. This could be:
- Standing at the center of their room
- Standing at a specific location that corresponds to a known spot in the virtual Makerspace
- Looking toward a specific direction that should align with the virtual Makerspace's forward direction

#### Step 2: Calibration Trigger
The user initiates calibration through the UI, which calls the calibration system.

#### Step 3: Alignment Calculation
The system calculates the transformation matrix by:
- Capturing the current XR Origin position and rotation
- Determining where this position should map to in the shared Makerspace space
- Computing the offset and rotation needed to align the spaces

#### Step 4: Matrix Application
The calculated transformation matrix is stored in the `CalibrationManager` and applied to all subsequent position updates.

### Working with the Calibration System

#### CalibrationManager.cs
The `CalibrationManager` handles:
- Storing the local-to-shared transformation matrix
- Providing methods to calculate and apply calibration
- Managing calibration state

Key methods:
```csharp
// Calculate calibration based on current position and target spawn point
public void CalibrateAtCurrentPosition(Vector3 targetSharedPosition, Quaternion targetSharedRotation)

// Apply calibration to transform local position to shared space
...
#### SharedSpaceManager.cs
The `SharedSpaceManager` works with `CalibrationManager` to:
- Store reference to the Makerspace root object
- Convert local XR poses using the calibration matrix
- Ensure all position updates use the shared coordinate system

#### PlayerRigTracker.cs
The `PlayerRigTracker` uses the calibrated transformation:
- Reads local headset and hand transforms from XR Origin
- Applies the calibration matrix to convert to shared space
- Updates the networked avatar positions in shared coordinates

### Calibration Workflow Example

1. **User joins the session**
   - Network spawns their PlayerAvatar
   - Local XR tracking starts
   - Calibration is initially uncalibrated

2. **User performs calibration**
   - User stands at their desired virtual position
   - User presses "Calibrate" button in UI
   - System captures current XR pose and calculates transformation

3. **Calibration is applied**
   - All subsequent position updates use the calibration matrix
   - User's avatar appears at correct position in shared Makerspace
   - Hand and head movements are properly aligned

4. **Continuous operation**
   - User moves around their physical space
   - Movements are tracked locally, transformed to shared space, and networked
   - Other users see the avatar moving correctly in the virtual Makerspace

### Calibration Best Practices

#### For Users
- Stand at a clear, open space when calibrating
- Face the direction you want to be your "forward" in the virtual space
- Keep the headset steady during calibration
- Recalibrate if you notice position drift or misalignment

#### For Developers
- Provide clear visual feedback during calibration
- Store calibration data for persistence across sessions
- Implement calibration validation to catch obvious errors
- Consider automatic recalibration if large position jumps are detected
- Add debugging tools to visualize the calibration transformation

### Common Calibration Issues

#### Position Offset
- **Cause**: Incorrect spawn point mapping or user not at expected position
- **Solution**: Verify target shared position matches user's intended virtual location

#### Rotation Misalignment
- **Cause**: User facing wrong direction during calibration
- **Solution**: Ensure user faces their intended "forward" direction when calibrating

#### Scale Mismatch
- **Cause**: Different room sizes or movement scales
- **Solution**: Consider adding scale factor to calibration matrix

#### Tracking Drift
- **Cause**: Headset tracking errors over time
- **Solution**: Implement periodic recalibration or drift correction

### Future Calibration Improvements

Planned enhancements to the calibration system:

1. **Visual Calibration Guide**: AR overlays showing where to stand and face
2. **Multi-point Calibration**: Using multiple reference points for better accuracy
3. **Automatic Calibration**: Detecting room layout and suggesting optimal calibration
4. **Persistent Calibration**: Saving calibration data for returning users
5. **Calibration Sharing**: Allowing calibrated users to help others calibrate
6. **Room Mapping**: Integrating with Quest's room scanning for better spatial understanding
